...
def kabsch(P, Q, max_rotation_radian=pi/4):
    ...
    U : matrix
        Rotation matrix (D,D)
    """
    # Arne here: Since we only need rotation of images in 2D
    # simply do that and then augment the matrix 
    # as to not break other calls
    P = [x[:2] for x in P]
    Q = [y[:2] for y in Q]
    ...
    # Still Arne: Pad the matrix back to 3D
    U = np.append(U,[[0], [0]], axis=1)
    U = np.append(U, [[0,0,1]], axis=0)
